...
test_expect_success 'Banner version check' 'test $(../graudit -v|cut -d" " -f 3|grep -F -f - ../graudit| grep Wireghoul|wc -l) -eq 1'
test_expect_code 1 'exclude files check' '../graudit -x *.js anotherfile'
test_expect_code 1 'include files check' '../graudit -n *.js anotherfile'
